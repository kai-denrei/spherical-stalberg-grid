...
| 7 | begun: first device screen (operator, 2026-09-03) found the ruler blind to `(pointer: coarse)` and no cue for tap-to-go. `?coarse=1` simulates the phone's blocks; the shell pins its geometry; tutorial wording + a three-mark coach. Checklist still to run in full. | `1cf2b02` |
