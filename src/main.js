// main.js — Three.js shell + dashboard around the sphere grid pipeline.
// All grid math lives in grid.js (Node-testable); this file only renders.
...
} from './grid.js?v=8ac44444';
import { mulberry32, randomSeed } from './rng.js?v=8ac44444';

// --- params (the dashboard) ------------------------------------------------
const params = {
  // generation
  seed: 42,
  points: 600,
...
// --- mesh <-> three glue ---------------------------------------------------
let mesh = null;        // pipeline mesh {vertices, quads, ...}
let edgeList = [];      // deduped [a,b] pairs for the wireframe
let cellColors = null;  // per-quad [r,g,b] for 'cells' mode
...
// pastel per-cell colors, deterministic from the seed
...
const errColorA = new THREE.Color(0x9fe2bf); // low error
const errColorB = new THREE.Color(0xe74c3c); // high error
...
function facePositionArray() {
  ...
  for (const q of quads) {
    // two triangles per quad: 0-1-2, 0-2-3 (quads stored outward-CCW)
...
function buildDefects() {
  ...
  const c3 = new THREE.Color(0xffb347);  // valence 3 — orange
  const c5 = new THREE.Color(0x47d7ff);  // valence 5 — cyan
  const cX = new THREE.Color(0xff47d7);  // valence 6+ / 2 — magenta
...
function defectPositionArray() {
  // pushed slightly off the surface so markers never z-fight the faces
...
// update position buffers in place after relax steps
...
// --- dashboard -------------------------------------------------------------
const gui = new GUI({ title: 'spherical stålberg grid' });
...
gGen.add(params, 'points', 50, 2000, 10).name('sample points').onFinishChange(regenerate);
