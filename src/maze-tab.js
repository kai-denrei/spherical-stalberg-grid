...
import { UNIT_NAMES, buildUnit, buildCreature, makeHeartCloud } from './units.js?v=7a96b990';
...
export function initMazeTab(root) {
  ...
  function spawnUnit() {
    ...
    const entry = {
      name: params.unit,
      ci,
      obj: buildCreature(params.unit, { walker: look().walker, walkerHi: look().walkerHi }),
    };
    placeSpawned(entry);
    scene.add(entry.obj);
    spawned.push(entry);
  ...
  function retintSpawned() {
    for (const entry of spawned) {
      ...
      entry.obj = buildCreature(entry.name, { walker: look().walker, walkerHi: look().walkerHi });
      placeSpawned(entry);
      scene.add(entry.obj);
    }
  }
